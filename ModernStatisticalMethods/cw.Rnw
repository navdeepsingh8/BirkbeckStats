...
\newpage
3. (a)
<<echo=TRUE,output=TRUE,fig=TRUE>>=
library(MASS)
...
<<echo=TRUE,output=FALSE,fig=TRUE>>=
...
<<eval=TRUE,echo=FALSE,output=FALSE,fig=FALSE>>=
niter <- 5000
p <- 0.4
lambda <- 20
x1 <- rep(0,niter)
n1 <- rep(0,niter)
n <- 60
#n is used to get the first value of x

set.seed(768)
for (i in 1:niter) {
    x <- rbinom(1,n,p);
    n <- rpois(1,(1-p)*lambda)+x;
    while(n==0) n <- rpois(1,(1-p)*lambda)+x;
    x1[i] <- x
    n1[i] <- n
}
@

<<echo=TRUE,output=TRUE,fig=FALSE>>=
...
\begin{figure}[h]
\begin{centering}
<<echo=FALSE,output=FALSE,fig=TRUE,height=7,width=13>>=
plot(x1,type="l",ylab="x",xlab="");
@
<<echo=FALSE,output=FALSE,fig=TRUE,height=7,width=13>>=
plot(n1,type="l",ylab="n",xlab="");
...
truehist(x1,xlab="x",nbin=20);
xseq<-min(x1):max(x1);
lines(xseq,dpois(xseq,lambda=p*lambda));
truehist(n1,xlab="n",nbin=20);
nseq<-min(n1):max(n1);
lines(nseq,dpois(nseq,lambda=lambda));
par(op);
@
...
perhaps 500 iterations, which yis a desirable result. The
