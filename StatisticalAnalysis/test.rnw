...
bark.dep
#Calculate mean, covariances and correlations
bark.stats=list();
...
\Sexpr{min(bark.stats$mean)}
,
\Sexpr{max(bark.stats$mean)}
...
\Sexpr{format(max(diag(bark.stats$cov)),digits=3)}
$ and E the lowest at $
\Sexpr{format(min(diag(bark.stats$cov)),digits=3)}
...
\Sexpr{format(100*bark.eig.cor.prop[1],digits=3)}
...
\Sexpr{format(100*bark.eig.cor.prop[2],digits=3)}
...
<<echo=FALSE,fig=TRUE>>=
op=par(pty="s");
plot(bark.eig.cov.prop,type="l",col=1,
     ylab="Proportion of variance",xlab="Principal component",lab=c(4,5,1));
points(bark.eig.cor.prop,type="l",col=1,lty=4,lwd=1.5);
legend("topright",legend=c("Covariance","Correlation"),
       col=1,lty=c(1,4),lwd=c(1,1.5),title="Component type");
grid();
par(op);
@
...
<<echo=FALSE>>=
age=matrix(seq(8,14,2));
dist.mean =matrix(c(22.88,23.81,25.72,27.47));
dist.cov  =matrix(c(6.017,2.292,3.629,1.613,
                    2.292,4.563,2.194,2.810,
                    3.629,2.194,7.032,3.241,
                    1.613,2.810,3.241,4.349),4);
dist.contr = cbind(c(-1,1,1,-1),c(-1,3,-3,1));
@
<<>>=
dist.mean
dist.cov
...
<<echo=FALSE,fig=TRUE>>=
op=par(pty="s");
plot(age,dist.mean,type="b",ylab="Measurement (mm)",xlab="Age (years)");
grid();
par(op);
@
...
\Sexpr{format(t.sq.obs.adj,digits=3)}
$ is smaller than the the $F_{2,14}(5\%)$ critical value of $
\Sexpr{format(f.crit,digits=3)}
