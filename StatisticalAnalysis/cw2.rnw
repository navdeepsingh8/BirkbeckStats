...
\Sexpr{ceil(odds["A"])} times more likely to survive than die in clinic A, whereas only about
\Sexpr{ceil(odds["B"])} times as likely to survive as die in clinic B. The corresponding odds ratio of
